...
class Solution(object):
    ...
    def numIslands(self, grid):
        ...
        for i in range(row):
            for j in range(col):
                if grid[i][j] == '1':
                    self.BFS(i, j, grid, row, col, island_num)
                    island_num += 1
        return island_num - 2

    def BFS(self, row, col, grid, row_num, col_num, island_num):
        ...
        while queue:
            ...
            for dire in direction:
                ...
                if 0 <= new_x < row_num and 0 <= new_y < col_num and grid[new_x][new_y] == '1':
                    ...
                    grid[new_x] = "".join(grid[new_x][:new_y]) + str(island_num) + "".join(grid[new_x][new_y + 1:])
